...
public class CommonMethods {
    ...
    // ����IP���У���
    public static boolean ComputeIPChecksum(IPv4 ipHeader) {
        short oldCrc = ipHeader.getHeaderChecksum();
        ipHeader.setHeaderChecksum((short) 0);// ����ǰ����
        short newCrc = CommonMethods.checksum(0, ipHeader.getByteBuffer().array(),
                0, ipHeader.getHeaderLength());
        ipHeader.setHeaderChecksum(newCrc);
        return oldCrc == newCrc;
    }

    // ����TCP��UDP��У���
    public static boolean ComputeTCPChecksum(IPv4 ipHeader, TCP tcpHeader) {
        ComputeIPChecksum(ipHeader);//����IPУ���
        int ipData_len = ipHeader.getTotalLength() - ipHeader.getHeaderLength();// IP��ݳ���
        if (ipData_len < 0)
            return false;
        // ����Ϊα�ײ���
        long sum = getsum(ipHeader.getByteBuffer().array(), 0
                + IPv4.offset_src_ip, 8);
        sum += ipHeader.getProtocol() & 0xFF;
        sum += ipData_len;

        short oldCrc = tcpHeader.getChecksum();
        tcpHeader.setChecksum((short) 0);// ����ǰ��0

        short newCrc = checksum(sum, tcpHeader.getByteBuffer().array(), 0, ipData_len);// ����У���

        tcpHeader.setChecksum(newCrc);
        return oldCrc == newCrc;
    }

    // ����TCP��UDP��У���
    public static boolean ComputeUDPChecksum(IPv4 ipHeader, UDP udpHeader) {
        ComputeIPChecksum(ipHeader);//����IPУ���
        int ipData_len = ipHeader.getTotalLength() - ipHeader.getHeaderLength();// IP��ݳ���
        if (ipData_len < 0)
            return false;
        ...
        short oldCrc = udpHeader.getCheckSum();
        udpHeader.setCheckSum((short) 0);// ����ǰ��0

        short newCrc = checksum(sum, udpHeader.getByteBuffer().array(), 0, ipData_len);// ����У���

        udpHeader.setCheckSum(newCrc);
        return oldCrc == newCrc;
    }
}
